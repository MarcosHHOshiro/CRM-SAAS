  ...
  return (
    <section className="rounded-[2rem] border border-dashed border-[var(--border)] bg-[var(--card)] p-8 text-center shadow-[var(--shadow-soft)]">
      ...
      <h2 className="mt-4 text-2xl font-semibold text-[var(--foreground)]">
        {hasFilters ? 'No leads match the current filters.' : 'No leads created yet.'}
      </h2>
      <p className="mx-auto mt-4 max-w-2xl text-sm leading-7 text-[var(--foreground-muted)]">
        {hasFilters
          ? 'Try broadening the search or clearing the status and owner filters.'
          : 'Start by creating your first lead to populate the pipeline and track the earliest opportunities.'}
      </p>
      {!hasFilters ? (
        <Link
