  ...
  return (
    <section className="rounded-lg border border-[var(--border)] bg-[var(--card)] p-6 shadow-[var(--shadow-soft)]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--card-dark)] px-5 py-4 text-center">
          <p className="text-xl font-semibold text-[var(--foreground)]">{totalTrackedOpportunities}</p>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-3">
        {pipelineSegments.map((segment) => (
          <article
            key={segment.key}
            className="rounded-lg border border-[var(--border)] bg-[var(--card-strong)] px-4 py-4"
          >
