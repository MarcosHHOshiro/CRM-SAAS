export function DashboardSkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      <section className="rounded-lg border border-[var(--border)] bg-[var(--card)] px-6 py-7 shadow-[var(--shadow-soft)]">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {Array.from({ length: 7 }).map((_, index) => (
          <article
            key={index}
            className="rounded-lg border border-[var(--border)] bg-[var(--card-strong)] p-5 shadow-[var(--shadow-soft)]"
          >
          ...
          >
            ...
            <div className="mt-6 space-y-3">
              {Array.from({ length: 3 }).map((__, itemIndex) => (
                <div key={itemIndex} className="rounded-lg border border-slate-100 bg-[var(--card-strong)] p-4">
                  <div className="h-4 w-24 rounded-full bg-slate-200" />
